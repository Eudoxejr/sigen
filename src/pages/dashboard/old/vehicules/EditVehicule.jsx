...
const EditVehicule = () => {
  ...
  const { id } = useParams();
  const [idTransport, setIdTransport] = useState(params?.vehicle?.transportMoyen?.idTransportMoyen);
  ...
  const { control, watch, setValue, handleSubmit, setError, formState: { errors, isDirty } } = useForm({
    ...
    defaultValues: {
      rental: params?.vehicle?.driver ? true : false,
      dailyVehiculeLocationCommission: params?.vehicle?.dailyVehiculeLocationCommission,
      AllClassOnVehicule: params?.vehicle?.AllClassOnVehicule?.map(c => c?.class?.idClass),
      infoVehicule: params?.vehicle?.infoVehicule?.map(i => {
        return {
            idField: i.idField,
      ...
      driverId: params?.vehicle?.driver?.idUser ? params?.vehicle?.driver?.idUser : undefined,
      startDate: params?.vehicle?.rentalCar?.startDate ? params?.vehicle?.rentalCar?.startDate.split("T")[0] : undefined,
      endDate: params?.vehicle?.rentalCar?.endDate ? params?.vehicle?.rentalCar?.endDate.split("T")[0] : undefined,
    }
  });

  //setValue("idTransportMoyen",idTransport);
  ...
  const { mutate, isLoading: mutateLoad } = useMutation({
    ...
    mutationFn: async (data) => {
        ...
        data.idVehicule = params?.vehicle?.idVehicule
        ...
        attachementArray.forEach((value,index) => {
            ...
            else if(((!value.file && !value.urlAttachement) || (value.file && (value.extension !== "png" && value.extension !== "jpeg" && value.extension !== "jpg" && value.extension !== "pdf"))))
            {
        ...
        return VehicleApi.updateVehicule(data)
  ...
  const getDriver = async (inputValue) => {
    ...
    let dataMaped = res.data.map((data) => { return { value: data?.idUser, label: data?.firstName + " " + data?.lastName, photoProfil: data?.photoProfil, email: data?.email, telephone: data?.telephone } })
    if(params?.vehicle?.driver?.idUser)
    {
        if(dataMaped.findIndex(d => d.value === params?.vehicle?.driver?.idUser) === -1)
        {
            dataMaped.push({
                value: params?.vehicle?.driver?.idUser, 
                label: params?.vehicle?.driver?.firstName + " " + params?.vehicle?.driver?.lastName, 
                photoProfil: params?.vehicle?.driver?.photoProfil, 
                email: params?.vehicle?.driver?.email, 
                telephone: params?.vehicle?.driver?.telephone
            })
        }
    }
    return dataMaped
  ...
  useEffect(() => {
    ...
    else
    {
      ...
      let newAttachementArray = params?.vehicle?.attachmentVehicule?.map(a => {
        return {
            idAttachement: a.idAttachement,
            urlAttachement: a.url,
            extension: a.extension,
            expirationDate: a.ExpirationDate ? a.ExpirationDate.split("T")[0] : undefined,
            file: undefined
        }
      })

      attachements.map((item) => {
        if(newAttachementArray.findIndex(a => a.idAttachement === item.idAttachement) === -1)
        {
            newAttachementArray.push({
        ...
        <Card>
          ...
          <CardBody className="p-4 pt-3 pb-[100px] h-[calc(100vh-210px)] overflow-auto shadow-none">
            ...
            <div className="w-full flex flex-col text-[14px] gap-[20px]">
              <p className="w-[700px] leading-[30px]">Pour modifier ce véhicule, veuillez remplir convenablement les différents champs présentés sur cette page puis cliquer sur le bouton <span className="font-bold text-black">[ Enregistrer le véhicule ]</span>. Notez que ce véhicule <span className="font-bold text-black">appartiennent à l'entreprise TPT973.</span></p>
              <p>Moyen de transport :</p>
              <div className="w-full flex items-center flex-wrap gap-[20px]">

                <Moyen className={"active"}>
                    <img alt={`Moyen de transport inconnu`} src={params?.vehicle?.transportMoyen?.illustrationPic} />
                    <div className="title">{params?.vehicle?.transportMoyen?.title}</div>
                </Moyen>
            ...
            {isLoading ?
              ...
              <>
                  ...
                  <div className="w-full flex flex-col text-[14px] gap-[20px] mt-[50px]">
                    ...
                    {attachements?.map((attachement, index) => (
                      <div className="w-[600px]" key={attachement.idAttachement}>
                        {((attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.urlAttachement || attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.file) && 
                        (attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.extension === "png" || attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.extension === "jpeg" || attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.extension === "jpg")) && 
                          <figure>
                            <img
                              className="h-96 w-full rounded-lg object-cover object-center border-solid border-2 border-green-600"
                              src={attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)].file ? URL.createObjectURL(attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)].file) : attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.urlAttachement}
                        ...
                        {((attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.urlAttachement || attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.file) && attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.extension === "pdf") && 
                          <div className="bg-green-600 text-white text-center rounded-lg border-solid border-2 border-green-800">
                            <a href={attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)].file ? URL.createObjectURL(attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)].file) : attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.urlAttachement} target="_blank">Ouvrir le fichier PDF : {attachement.title}</a>
                          </div>
                        }
                        <br/>
                        <Input
                        ...
                        <Input 
                          ...
                          defaultValue={attachementArray[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]?.expirationDate}
                          onChange={(e) => {
                            let attachementIndex = attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)
                            let newAttachementArray = Array.from(attachementArray);
                          ...
                          name={attachement.idAttachement + "#e" +attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)}
                        />
                        {fileErrors[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)] &&
                          <span className=" text-[11px] text-red-400 mt-1" >{fileErrors[attachementArray.findIndex(a => a.idAttachement === attachement.idAttachement)]}</span>
                        }
                    </div>
                    ))}
                  </div>
                ...
                {fields.length > 0 &&
                  <div className="w-full flex flex-col text-[14px] gap-[20px] mt-[50px]">
                    <p>Nous avons besoin de quelques informations supplémentaires pour modifier ce véhicule. Veuillez remplir les champs ci-dessous :</p>
                        ...
                        render={({
                          ...
                          <div className="w-[600px]">
                            <Input
                              ...
                              defaultValue={params?.vehicle?.infoVehicule ? params?.vehicle?.infoVehicule[params?.vehicle?.infoVehicule?.findIndex(i => i.idField === field.idField)]?.value : undefined}
                              value={value}
                            />
                            {error &&
                              <span className=" text-[11px] text-red-400 mt-1" >{error.message}</span>
                        ...
                        render={({
                          ...
                          <div className="w-[600px]">
                            <AsyncSelect
                              cacheOptions
                              defaultOptions
                              defaultValue={params?.vehicle?.driver?.idUser ? {
                                value: params?.vehicle?.driver?.idUser, 
                                label: params?.vehicle?.driver?.firstName + " " + params?.vehicle?.driver?.lastName, 
                                photoProfil: params?.vehicle?.driver?.photoProfil, 
                                email: params?.vehicle?.driver?.email, 
                        ...
                        <Controller
                          render={({
                            ...
                            <div className="w-[270px]">
                              <Input
                                label="Date de début"
                                defaultValue={params?.vehicle?.rentalCar?.startDate ? params?.vehicle?.rentalCar?.startDate.split("T")[0] : undefined}
                                onChange={onChange}
                                type="date" color="blue-gray" size="lg"
                              />
                              {error &&
                        ...
                        <Controller
                          render={({
                            ...
                            <div className="w-[270px]">
                              <Input
                                label="Date de fin"
                                defaultValue={params?.vehicle?.rentalCar?.endDate ? params?.vehicle?.rentalCar?.endDate.split("T")[0] : undefined}
                                onChange={onChange}
                                type="date" color="blue-gray" size="lg"
                              />
                              {error &&
