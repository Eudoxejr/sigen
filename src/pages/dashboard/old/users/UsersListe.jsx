...
import { columnUser } from "@/utils/columsDatatable";
...
export function UsersListe() {
  ...
  const { isLoading, refetch } = useQuery({
    queryKey: ["getUsersFilter", 1, 25, searchTerm],
    queryFn: async ({ queryKey }) => {
      return DriverApi.getDriver(queryKey[1], queryKey[2], ["USER"], queryKey[3])
    },
    onSuccess: (data) => {
      queryClient.setQueriesData(["getUsers"], (dataUser) => {
  ...
  return (
    <RenderIf allowedTo={Permissions.VIEW_USERS_LIST}>
          ...
          <CardBody className="md:h-[calc(100vh-220px)] shadow-none flex flex-col px-4 pt-0 pb-4 gap-[15px] overflow-auto">
            ...
            <div className="flex-1 w-full">
              <DataGridComponent
                ...
                columns={columnUser}
                queryFirstKey={"getUsers"}
                fnQuery={async ({ queryKey }) => {
                  return DriverApi.getDriver(queryKey[1], queryKey[2], ["USER"])
                }}
                idpri={"idUser"}
                handleTotal={updateTotal}
                loadSup={
                  typeof (searchTerm) == 'string' ? isLoading : false
                }
              />
            </div>
          </CardBody>
