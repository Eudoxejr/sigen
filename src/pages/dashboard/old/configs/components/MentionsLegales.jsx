...
const MentionsLegales = ({
  ...
  const handleSubmit = async () => {
    if (!!htmlToText(htmlStr)?.trim()) {
      setLoading(true);
      await ConfigsApi.updateMentionsLegales(htmlStr)
        .then(({ data: response }) => {
          if (response?.success) {
            toast.success("Les mentions légales ont été mises à jour !");
          } else {
            toast.error("Une erreur est survenue lors de l'enregistrement des mentions légales");
          }
        })
        .catch(_ => {
          toast.error("Une erreur est survenue lors de l'enregistrement des mentions légales");
  ...
  const handleFetchMentions = async () => {
    setFetching(true);
    await ConfigsApi.fetchHtmlConfig("mention-legal")
      .then(({ data: response }) => {
        if (response?.success) {
