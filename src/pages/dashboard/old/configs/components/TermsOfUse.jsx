...
const TermsOfUse = ({
  ...
  const handleSubmit = async () => {
    if (!!htmlToText(htmlStr)?.trim()) {
      setLoading(true);
      await ConfigsApi.updateCGU(htmlStr)
        .then(({ data: response }) => {
          if (response?.success) {
            toast.success("Les conditions générales d'utilisation ont été mises à jour !");
          } else {
            toast.error("Une erreur est survenue lors de l'enregistrement des conditions générales d'utilisation");
          }
        })
        .catch(_ => {
          toast.error("Une erreur est survenue lors de l'enregistrement des conditions générales d'utilisation");
  ...
  const handleFetchCGU = async () => {
    setFetching(true);
    await ConfigsApi.fetchHtmlConfig("cgu")
      .then(({ data: response }) => {
        if (response?.success) {
