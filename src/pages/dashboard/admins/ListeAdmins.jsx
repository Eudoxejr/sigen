...
import { adminsColumns } from '@/utils/columsDatatable';
...
const ListeAdmins = () => {
  ...
  const { isLoading, refetch } = useQuery({
    queryKey: ["getFilteredAdminList", 1, 25, searchTerm],
    queryFn: async ({ queryKey }) => {
      return AdminsApi.fetchAllAdmins(queryKey[1], queryKey[2], queryKey[3]);
    },
    onSuccess: (data) => {
      queryClient.setQueriesData(["getAdmins"], (dataUser) => {
        ...
        <Card>
          ...
          <CardBody className="md:h-[calc(100vh-220px)] shadow-none flex flex-col px-4 pt-0 pb-4 gap-[15px] overflow-auto">
            <div className="w-full flex justify-center items-center gap-[10px] text-[14px]">
              ...
              {isAllowedTo(Permissions.ADD_ADMIN) &&
                <button
            ...
            <div className="flex-1 w-full">
              <DataGridComponent
                ...
                fnQuery={({ queryKey }) => AdminsApi.fetchAllAdmins(queryKey[1], queryKey[2], queryKey[3])}
                handleTotal={setTotal}
                queryFirstKey="getAdmins"
