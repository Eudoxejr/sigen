...
const NextedInformationDone = ({nextIndex, control,}) => {

    const { fields:fieldsInfoDone } = useFieldArray({
        control, // control props comes from useForm (optional: if you are using FormContext)
        name: `subCategories[${nextIndex}].informationDone` // unique name for your Field Array
    });
            ...
            {fieldsInfoDone.map((field, index) => (
                ...
                <fieldset key={field.id} className=" flex flex-row gap-x-4 gap-y-5 w-[95%] flex-wrap " >
                    ...
                    <Controller
                        ...
                        name={`subCategories[${nextIndex}].informationDone[${index}].value`}
                        control={control}
                    />
...
export default NextedInformationDone;
